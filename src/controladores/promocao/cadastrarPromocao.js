...
const cadastrarPromocaoProduto = async (req, res) => {
    const { descricao, preco_promocional, produto_id, horario_inicio, horario_fim } = req.body;
    try {
        const promocao = await knex('promocoes')
            .insert({ descricao, preco_promocional, produto_id, horario_inicio, horario_fim })
            .returning('*');

        await knex('produtos').update({ promocao: true }).where({ id: produto_id });
